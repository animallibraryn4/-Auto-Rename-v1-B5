...
async def ban_user_cmd(client, message):
    ...
    # Check if the target user is the bot admin
    if user_id in ADMIN_USER_ID:
        return await message.reply_text("Admin cannot be banned.")
    ...
    success = await codeflixbots.ban_user(user_id, reason)
    
    if success:
        # Try to notify the user they were banned
        try:
            await client.send_message(user_id, f"You have been banned from using this bot by an admin.\n\nReason: **{reason}**\n\n{Txt.BAN_MSG}")
        except Exception:
            pass  # Ignore if the message fails to send

        return await message.reply_text(f"User `{user_id}` has been **BANNED**.\nReason: **{reason}**")
    else:
        return await message.reply_text(f"Failed to ban user `{user_id}`. Database error.")

# Unban User Command
@Client.on_message(filters.private & filters.command("unban") & filters.user(ADMIN_USER_ID))
async def unban_user_cmd(client, message):
    ...
    if success:
        # Try to notify the user they were unbanned
        try:
            await client.send_message(user_id, "You have been **UNBANNED** and can now use the bot again.")
        except Exception:
            pass  # Ignore if the message fails to send

        return await message.reply_text(f"User `{user_id}` has been **UNBANNED**.")
    else:
        return await message.reply_text(f"Failed to unban user `{user_id}`. Database error.")
